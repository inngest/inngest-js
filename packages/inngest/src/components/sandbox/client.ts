...
const outputResponseSchema = z
  .object({ chunks: z.array(wireOutputChunkSchema) })
  .strict();
...
class SandboxRestTransport {
      ...
      pull: async (controller) => {
        try {
          ...
          if (
            ...
            const error = parseWithSchema(
              errorItemSchema,
              (frame as { error?: unknown }).error,
  ...
  private errorFromItem(
    ...
    const ambiguous = item.code === "operation_ambiguous";
    const availability =
    ...
    refresh: async () => {
      try {
        ...
        return createDirectProcessFacade(
          sandboxProcessRefFromResource(ref.sandboxId, envelope?.data),
          transport,
        );
      } catch (error) {
        if (
...
const createDirectSandboxFacade = (
  ...
  const facade: Sandbox = {
    ...
    processes: Object.freeze({
      ...
      list: async () => {
        ...
        const processes = parseWithSchema(
          z.array(sandboxProcessResourceSchema),
          envelope?.data,
    ...
    refresh: async () => {
      try {
        const { envelope } = await transport.json("get", "GET", basePath, {
          statuses: [200],
          sandboxId: ref.id,
        });
