...
import { createV1InngestExecution, _internals as v1Internals } from "./execution/v1";
...
describe("StepFailed OpCode with try/catch", () => {
  ...
  test("step that throws NonRetriableError should be catchable in try/catch", async () => {
    ...
    const stepHashedId = "5703c5614966d8141bbdd399c401f7232b938c47"; // hash of "failing-step"
  ...
  test("step that exhausts retries should be catchable in try/catch", async () => {
    ...
    const fn = inngest.createFunction(
      ...
      { event: "test/event" },
      async ({ step, attempt, maxAttempts }) => {
        try {
    ...
    const stepHashedId = "9ca8a026b5a0db79eefa19bc9f82a892694d69d1"; // hash of "failing-step-2"
    const execution = createV1InngestExecution({
      client: inngest,
      fn: fn as InngestFunction.Any,
      data: {
  ...
  test("data with name/message that is not a serialized error resolves on resume", async () => {
    ...
    if (result.type === "function-resolved") {
      ...
      fail(`Unexpected result type: ${result.type}`);
    }
  });

  test("memoized resume rejects when data is a serialized error", async () => {
    let caughtError = false;

    const fn = inngest.createFunction(
      { id: "test-serialized-error-data", retries: 1 },
      { event: "test/event" },
      async ({ step }) => {
        try {
          await step.run("serialized-error", () => {
            return "should not be returned";
          });
          return "unreachable";
        } catch {
          caughtError = true;
          return "Handled serialized error in data";
        }
      }
    );

    const stepHashedId = v1Internals.hashId("serialized-error");
    const execution = createV1InngestExecution({
      client: inngest,
      fn: fn as InngestFunction.Any,
      data: {
      ...
      stepState: {
        [stepHashedId]: {
          id: stepHashedId,
          data: serializeError(new Error("Serialized failure")),
          error: undefined,
          seen: false,
          fulfilled: false,
        },
    ...
    expect(caughtError).toBe(true);
    if (result.type === "function-resolved") {
      expect(result.data).toBe("Handled serialized error in data");
    } else if (result.type === "step-ran") {
