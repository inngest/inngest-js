...
The `LazyOps` class (`lazyOps.ts`) is just a buffer: `push(op)` appends, `drain()` takes and clears, `has()` peeks. The engine owns op construction and shipping. Drain sites: `checkpoint()`, `maybeReturnNewSteps`, the `steps-found` handler, and the async `function-resolved` / `function-rejected` handlers.
