...
testFramework("Astro", AstroHandler, {
  ...
  transformReq: (req) => {
    ...
    // biome-ignore lint/complexity/noForEach: We do not check for complexity here
    Object.entries(req.headers).forEach(([k, v]) => {
      headers.set(k, v as string);
    });
    // biome-ignore lint/suspicious/noExplicitAny: We do not check for complexity here
