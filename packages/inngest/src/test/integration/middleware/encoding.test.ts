...
test("base64 encoding/decoding middleware", async () => {
  ...
  class EncodingMiddleware extends BaseSerializerMiddleware<Serialized> {
    ...
    // Override so we can capture the pre-transformed runInfo
    override transformRunInput(arg: Middleware.TransformRunInputArgs) {
      state.transformRunCalls.push(arg.runInfo);
      return super.transformRunInput(arg);
    }
  }
  ...
  expect(state.transformRunCalls).toEqual([
    expectedRunInfo,
    {
      ...expectedRunInfo,
      steps: {
