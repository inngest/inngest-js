...
export function SimpleSearch() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      while (true) {
        ...
        setUpdates((prev) => [...prev, JSON.parse(text).data]);
      }
    } catch (error) {
  ...
  const handleFloatingInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    setFloatingInput(e.target.value);
  };
 
  return (
    <div className="max-w-3xl mx-auto bg-gray-50 min-h-screen flex flex-col">
      <div className="p-6">
        <div className="bg-white p-6 rounded-lg border border-gray-200 shadow-sm mb-6">
          <h2 className="text-xl font-bold text-gray-900">AgentKit Web Search</h2>
        </div>
